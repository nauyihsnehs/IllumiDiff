...
class BaseDataModule(L.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train_data, batch_size=self.batch_size, drop_last=True, shuffle=True, num_workers=4, persistent_workers=True)
...
class SGNetDataset(Dataset):
    def __init__(self, rgb_path, input_ls_path=None, sg_path=None, env_path=None, resolution=(256, 256), id_net_ckpt_path=None):
        input_list = [p.as_posix() for p in pathlib.Path(rgb_path).glob('*.*')]
        input_list.sort()
    ...
    def __getitem__(self, idx):
        ...
        ldr_img_path = [p.as_posix() for p in pathlib.Path(self.rgb_path).glob(f'*{img_name}.*')][0]
        ...
        if self.input_ls_path is None:
            ...
            mask_img = cv.imread(f'{self.input_ls_path}/{img_name}.png')
        ...
        # testing
        if self.env_path is None: return ldr_img, mask_img, img_name
        ...
        env_gt_clip[lum < 1] = 0.
        env_gt = np.log10(env_gt + 1)
        env_gt_clip = np.log10(env_gt_clip + 1)
...
class SGNetDataModule(BaseDataModule):
    def __init__(self, base_path, input_path, input_ls_path=None, hdr_path=None, sg_path=None, resolution=256,
                 batch_size=1, id_net_ckpt_path=None):  # , model_type, frames=24
        super().__init__()
        self.predict_data, self.test_data, self.val_data, self.train_data = None, None, None, None
        self.data_num = None
        self.input_path = base_path + '/' + input_path
        if input_ls_path is not None: self.input_ls_path = base_path + '/' + input_ls_path
    ...
    def setup(self, stage: str):
        ...
        if stage == "predict":
            mtds = SGNetDataset(self.input_path, self.input_ls_path, resolution=self.resolution, id_net_ckpt_path=self.id_net_ckpt_path)
            self.predict_data = mtds
            self.data_num = len(self.predict_data)


class ASGNetDataset(Dataset):
    def __init__(self, input_path, ldr_path=None, asg_path=None, resolution=(512, 256)):
        input_list = [p.as_posix() for p in pathlib.Path(input_path).glob('*.*')]
        input_list.sort()
        self.input_list = input_list
    ...
    def __getitem__(self, idx):
        ...
        if self.ldr_path is None or self.asg_path is None: return ldr_img, img_name
...
class ASGNetDataModule(BaseDataModule):
    def __init__(self, base_path, input_path, ldr_path=None, asg_path=None, resolution=(512, 256),
                 batch_size=1):  # , model_type, frames=24
        super().__init__()
        self.data_num = None
        self.predict_data, self.test_data, self.val_data, self.train_data = None, None, None, None
        self.input_path = base_path + '/' + input_path
...
class HDRNetDataset(Dataset):
    def __init__(self, ldr_path, sg_path=None, ls_path=None, hdr_path=None, resolution=(512, 256)):
        input_list = [p.as_posix() for p in pathlib.Path(sg_path).glob('*.*')]
        input_list.sort()
        self.input_list = input_list
        self.ldr_path = ldr_path
        self.sg_path = sg_path
    ...
    def __getitem__(self, idx):
        ...
        ldr_img_path = [p.as_posix() for p in pathlib.Path(self.ldr_path).glob(f'*{img_name}.*')][0]
        ldr_img = cv.imread(ldr_img_path)
        ...
        hdr_gt_mask = np.ones_like(hdr_gt)
        hdr_gt_mask[hdr_gt == 0.] = 0.
        hdr_gt = np.log10(hdr_gt + 1)
        hdr_gt = torch.from_numpy(hdr_gt).float().permute(2, 0, 1)
        hdr_gt_mask = torch.from_numpy(hdr_gt_mask).float().permute(2, 0, 1)

        return ldr_img, sg_img, mask_img, hdr_gt, hdr_gt_mask, img_name


class HDRNetDataModule(BaseDataModule):
    def __init__(self, base_path, input_path, sg_path=None, ls_path=None, hdr_path=None, resolution=(512, 256), batch_size=1):
        # , model_type, frames=24
        super().__init__()
        self.predict_data, self.test_data, self.val_data, self.train_data = None, None, None, None
        self.data_num = None
        self.input_path = base_path + '/' + input_path
        self.sg_path = base_path + '/' + sg_path
        self.ls_path = base_path + '/' + ls_path
        if hdr_path: self.hdr_path = base_path + '/' + hdr_path
        self.resolution = resolution
        self.batch_size = batch_size
        self.save_hyperparameters()
    ...
    def setup(self, stage: str):
        ...
        if stage == "predict":
            mtds = HDRNetDataset(self.input_path, self.sg_path, self.ls_path, resolution=self.resolution)
            self.predict_data = mtds
            self.data_num = len(self.predict_data)
...
class IDNetDataset(Dataset):
    def __init__(self, ldr_path, ls_path=None, resolution=(512, 256)):
        input_list = [p.as_posix() for p in pathlib.Path(ldr_path).glob('*.*')]
        input_list.sort()
        self.input_list = input_list
        self.ldr_path = ldr_path
        self.ls_path = ls_path
...
class IDNetDataModule(BaseDataModule):
    def __init__(self, base_path, input_path, ls_path=None, resolution=(512, 256),
                 batch_size=1):  # , model_type, frames=24
        super().__init__()
        self.predict_data, self.test_data, self.val_data, self.train_data = None, None, None, None
        self.data_num = None
