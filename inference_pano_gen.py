...
def run_sampler(
        ...
        img_name, output_path, batch: int = 4,
        seed=-1,
        strength=1.0,
        ddim_steps=50,
        eta=0.,
...
if __name__ == '__main__':
    ...
    model_path = 'ckpts/control-epoch=9-step=47950.ckpt'
    epoch, step = map(int, __import__('re').search(r'epoch=(\d+)-step=(\d+)', model_path).groups())
    model_name = f'e{epoch:02d}_s{step:06d}'
    cmodel.load_state_dict(load_state_dict(model_path, location='cuda'), strict=False)
    cmodel.eval()
    ...
    for input_path in tqdm(input_paths):
        ...
        in_pano, _ = pers2pano(in_img, pano_res, vfov=90, rotation_ear=[0, 0, 0])
        _, in_mask = pers2pano(in_img, pano_res, vfov=88, rotation_ear=[0, 0, 0])
        in_mask[in_mask <= 0.5] = 0
        in_mask[in_mask > 0.5] = 1
        in_mask = torch.from_numpy(in_mask).float().cuda()[..., None]
        ...
        in_asg = in_asg * (1 - in_mask) + in_pano * in_mask
        ...
        run_sampler(cmodel, in_pano, in_asg, in_sg, in_img, save_name, save_path, seed=3407)
